function [cfg] = checkParameters(cfg)
    ...
    fieldsToSet.timing.endDelay = 14;
...
function fieldsToSet = setKeyboards(fieldsToSet)
    ...
    fieldsToSet.keyboard.responseKey = {'a', 'b', 'c', 'd'};
...
function cfg = setMRI(cfg)
    ...
    fieldsToSet.mri.triggerKey = 's';

    fieldsToSet.mri.triggerNb = 1;

    fieldsToSet.mri.repetitionTime = 1.75;
...
function fieldsToSet = setMonitor(fieldsToSet)
    ...
    if strcmpi(fieldsToSet.testingDevice, 'mri')
        fieldsToSet.screen.monitorWidth = 69.8;
        fieldsToSet.screen.monitorDistance = 170;
    end
